...
#define DI_LOG_TRACE(...) ::DI::Log::getConsoleEngine()->trace(__VA_ARGS__);\
                          ::DI::Log::_logData->buffer.push_back(std::pair<std::string,glm::vec4>(fmt::format(__VA_ARGS__),glm::vec4(1.0)))

#define DI_LOG_INFO(...)  ::DI::Log::getConsoleEngine()->info(__VA_ARGS__);\
                          ::DI::Log::_logData->buffer.push_back(std::pair<std::string,glm::vec4>(fmt::format(__VA_ARGS__),glm::vec4(0.0,1.0,0.0,1.0)))

#define DI_LOG_WARN(...)  ::DI::Log::getConsoleEngine()->warn(__VA_ARGS__);\
                          ::DI::Log::_logData->buffer.push_back(std::pair<std::string,glm::vec4>(fmt::format(__VA_ARGS__),glm::vec4(1.0,1.0,0.0,1.0)))

#define DI_LOG_ERROR(...) ::DI::Log::getConsoleEngine()->error (__VA_ARGS__);\
                          ::DI::Log::_logData->buffer.push_back(std::pair<std::string,glm::vec4>(fmt::format(__VA_ARGS__),glm::vec4(1.0,0.0,0.0,1.0)))

#define DI_LOG_DEBUG(...) ::DI::Log::getConsoleEngine()->debug (__VA_ARGS__);\
                          ::DI::Log::_logData->buffer.push_back(std::pair<std::string,glm::vec4>(fmt::format(__VA_ARGS__),glm::vec4(0.0,0.0,1.0,1.0)))
