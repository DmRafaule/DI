...
MyScene::MyScene(){
    ...
    DI::ModelHandler::Load(*simpleSphere,"res/models/europe/europe.dae");
    for (int i = 0; i < simpleSphere->meshes.size(); ++i){
        DI::MeshHandler::Set(*simpleSphere->meshes[i]);
...
void MyScene::RenderScene(){
    ...
    DI::ModelHandler::Translate(*simpleSphere,glm::vec3(0.0f,0.0f,0.0f));
